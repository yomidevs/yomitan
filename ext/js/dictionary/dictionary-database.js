...
export class DictionaryDatabase {
        ...
        if (this._bulkImportDepth === 0) {
            ...
            try {
                ...
                if (this._importDebugLogging) {
                    log.log(
                        '[yomitan-db-import] finishBulkImport ' +
        ...
        try {
            ...
            for (let i = start, ii = start + count; i < ii; ++i) {
                ...
                if (this._importDebugLogging && (tNow - lastProgressLog) >= this._termBulkAddLogIntervalMs) {
                    ...
                    log.log(
                        `[yomitan-db-import] bulkAdd terms progress rows=${processedRowCount}/${count} ` +
            ...
            if (this._importDebugLogging) {
                ...
                log.log(
                    `[yomitan-db-import] bulkAdd terms done rows=${count} total=${totalMs.toFixed(1)}ms ` +
    ...
    async _bulkAddTermsWithoutContentDedup(items, start, count) {
        ...
        try {
            ...
            if (this._importDebugLogging) {
                log.log(
                    `[yomitan-db-import] bulkAdd terms no-dedup contentAppend=${contentAppendMs.toFixed(1)}ms ` +
    ...
    async _ensureTermsVirtualTable() {
        ...
        } else if (isVirtualTerms && !termsSql.includes('YOMITAN_TERMS')) {
            db.exec('DROP TABLE terms');
        }
        db.exec(`
            CREATE VIRTUAL TABLE IF NOT EXISTS terms USING yomitan_terms(
                dictionary,
                expression,
                reading,
                expressionReverse,
    ...
    _registerTermsVirtualTableModule() {
        ...
        const rc = capi.sqlite3_create_module(dbPointer, 'yomitan_terms', typedModule, 0);
        if (rc !== 0) {
            throw new Error(`Failed to register yomitan_terms module: rc=${rc}`);
        }
