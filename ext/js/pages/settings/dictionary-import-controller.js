...
import {getRecommendedDictionaryDescription} from '../../data/recommended-dictionary-i18n.js';
...
export class DictionaryImportController {
    ...
    async _renderRecommendedDictionaries() {
        ...
        const nameToKey = await this._loadRecommendedDictionaryI18nKeys();
        ...
        for (const {property, element} of recommendedDictionaryCategories) {
            this._renderRecommendedDictionaryGroup(
                recommendedDictionaries[language][property],
                element,
                installedDictionaryNames,
                installedDictionaryDownloadUrls,
                nameToKey,
            );
        }
    ...
    /**
     * Map of dictionary name → chrome.i18n message key (rec_dict_desc_*).
     * Generated by dev/bin/i18n-sync-recommended-dicts.js
     * @returns {Promise<?Record<string, string>>}
     */
    async _loadRecommendedDictionaryI18nKeys() {
        try {
            const url = '../../data/recommended-dictionaries-i18n-keys.json';
            const response = await fetch(url, {
                method: 'GET',
                mode: 'no-cors',
                cache: 'default',
     ...
     */
    _renderRecommendedDictionaryGroup(recommendedDictionaries, dictionariesList, installedDictionaryNames, installedDictionaryDownloadUrls, nameToKey) {
        ...
        for (const dictionary of recommendedDictionaries) {
            if (dictionariesList) {
                ...
                description.textContent = getRecommendedDictionaryDescription(
                    dictionary.name,
                    nameToKey,
