 ...
 * @param {{name: string, message: string, stack: string, data?: *}|{value: *, hasValue: boolean}|chrome.runtime.LastError} serializedError A simple object which was initially generated by serializeError.
 * @returns {Error|*} A new `Error` instance.
 */
export function deserializeError(serializedError) {
    if ('hasValue' in serializedError && serializedError.hasValue) {
        return serializedError.value;
    }
    if ('message' in serializedError) {
        const error = new Error(serializedError.message);
        if ('name' in serializedError) {
            error.name = serializedError.name;
        }
        if ('stack' in serializedError) {
            error.stack = serializedError.stack;
        }
        if ('data' in serializedError) {
 ...
 * @returns {{promise: Promise, resolve: (value: any) => void, reject: Function}} An object `{promise, resolve, reject}`, containing the promise and the resolve/reject functions.
 */
 ...
 * @returns {ExtendedPromise} A promise with two additional properties: `resolve` and `reject`, which can be used to complete the promise early.
 */
